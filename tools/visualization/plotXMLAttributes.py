...
def getOptions(args=None):
    ...
    optParser.add_option("-i", "--idattr",  default="id",
    ...
    optParser.add_option(
        "--idelem",  help="element for grouping data points into lines")
    optParser.add_option("-s", "--show", action="store_true",
                         default=False, help="show plot directly")
    optParser.add_option("--csv-output", dest="csv_output",
                         help="write plot as csv", metavar="FILE")
    ...
    optParser.add_option(
        "--xfactor", help="multiplier for x-data", type=float, default=1)
    optParser.add_option(
        "--yfactor", help="multiplier for y-data", type=float, default=1)
    ...
    optParser.add_option("--legend", action="store_true",
                         default=False, help="Add legend")
    ...
    if options.barplot:
        if options.xbin is None:
            ...
            if options.verbose:
                print(
                    "Binning set to %s for barplot. Use option --xbin to set a custom value." % options.xbin)
...
def onpick(event):
    ...
    print("dataID=%s x=%d y=%d" %
          (event.artist.get_label(), mevent.xdata, mevent.ydata))
...
def getDataStream(options):
    ...
    if len(allElems) == 2:
        ...
        sys.exit(
            "Found attributes at elements %s but at most 2 elements are supported" % allElems)
            ...
            if options.barplot or options.hbarplot:
                if options.barplot:
                    if numericXCount > 0:
                        ...
                        center = [x + barOffset *
                                  barWidth for x in range(len(xvalues))]
                    ...
                    if numericYCount > 0:
                        ...
                        center = [y + barOffset *
                                  barWidth for y in range(len(yvalues))]
                ...
                if options.scatterplot or (min(yvalues) == max(yvalues) and min(xvalues) == max(xvalues)):
                    ...
                    if marker is None:
                        marker = 'o'
                plt.plot(xvalues, yvalues, linestyle=linestyle,
                         marker=marker, picker=True, label=dataID)
